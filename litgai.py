...
if st.session_state.page == "🏠 Home":
    ...
    with col2:
        min_temperature = st.number_input("最小温度 (K) ", min_value=300.0, max_value=700.0, format="%.1f", value=None,help="请输入 300 - 700 K 之间的数值")
    ...
    with col_pres_temp[0]:
        pressure = st.number_input(
            "输入压力 (MPa) ",
            min_value=40.0,          # 最小压力设为40
            max_value=200.0,         # 最大压力设为200
            step=5.0,
            format="%.2f",
            value=None,              # 默认值为40
            help="请输入 40 - 200 MPa 之间的数值"
        )
    with col_pres_temp[1]:
        temperature = st.number_input(
            "输入温度 (K) ",
            min_value=300.0,         # 最小温度设为300
            max_value=700.0,         # 最大温度设为700
            step=5.0,
            format="%.1f",
            value=None,            # 默认值为300
            help="请输入 300 - 700 K 之间的数值"
        )
